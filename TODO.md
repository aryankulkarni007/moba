...
- [x] **Rounding direction - DECIDED: floor everywhere.** Comments in `fx.hpp`
      and `fx64.hpp` are updated and describe the chosen policy. **Code edits
      still pending** (owner: Aryan): add `detail::fdiv` and route the seven
      division sites through it - `fx.hpp` `from_ratio`, `operator/(fx)`,
      `operator/(i32)`, `div_sat`; `fx64.hpp` `operator/(fx64)`,
      `operator/(i32)`, `div_sat`. Until those land, the comments describe
      behaviour the code does not yet have.
...
- [x] `fx/fx64.hpp` `narrow()` - measured, not broken. `fx::operator*`,
      ...
      negatives across 6.8M products. Comment updated; the test that holds them
      there is now tracked in P3.

- [x] `fx/fx64.hpp` `operator/(fx)` - no action. The existing note already
      resolves it: no caller exists, do not add it. Retag `[decide]` →
      `[sequencing]` when convenient so it stops reading as an open question.
...
- [ ] `fx/tests/CMakeLists.txt:4` - `test_fx64.cpp`: `mul_wide` exactness,
      `narrow()` saturation, and the four-path rounding agreement promoted from
      P1 - `fx::operator*` vs `narrow(mul_wide)` vs `narrow(fx64*fx64)` vs
      `narrow(fx64*fx)`, swept over negatives.
